    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                ...
                continue
            raise RuntimeError(f"Failed to call createTask endpoint after 3 attempts: {exc}") from exc
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                time.sleep(2 ** attempt * 3)
                continue
            raise RuntimeError(f"Failed to call recordInfo endpoint after 3 attempts: {exc}") from exc
        except requests.RequestException as exc:
