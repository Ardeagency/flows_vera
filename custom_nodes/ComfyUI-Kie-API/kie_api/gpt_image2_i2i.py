"""GPT Image 2 image-to-image helper (via Kie.ai)."""
...
def run_gpt_image2_image_to_image(
    ...
    # Retry loop on TransientKieError (upstream OpenAI timeouts return failCode=500).
    attempts = MAX_RETRIES_ON_TRANSIENT + 1
    last_transient: Exception | None = None
    for attempt in range(1, attempts + 1):
        _log(log, f"Sending {len(input_urls)} image URL(s) to createTask (attempt {attempt}/{attempts})")
        _log(log, "Creating GPT Image 2 image-to-image task...")
        ...
        try:
            ...
            if attempt >= attempts:
                ...
                raise
            backoff = RETRY_BACKOFF_S * attempt  # 8s, 16s
    ...
    raise RuntimeError("GPT Image 2 I2I exhausted retry loop without success or exception.")
